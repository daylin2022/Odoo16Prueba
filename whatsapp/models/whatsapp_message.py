...
class WhatsAppMessage(models.Model):
    ...
    def _gc_whatsapp_messages(self):
        ...
        inside mail.channel / mail.messages (as every other discussions).
    ...
    def _update_message_fetched_seen(self):
        ...
        if self.mail_message_id.model != 'mail.channel':
            return
        channel = self.env['mail.channel'].browse(self.mail_message_id.res_id)
        ...
        if self.state == 'read':
            ...
            notification_type = 'mail.channel.member/seen'
        elif self.state == 'delivered':
            channel_member.write({'fetched_message_id': self.mail_message_id.id})
            notification_type = 'mail.channel.member/fetched'
