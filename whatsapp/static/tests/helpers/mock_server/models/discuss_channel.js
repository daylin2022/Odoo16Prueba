...
patch(MockServer.prototype, {
    async _performRPC(route, args) {
        if (args.model === "mail.channel" && args.method === "whatsapp_channel_join_and_pin") {
    ...
    _mockDiscussChannelJoinAndPin(ids) {
        ...
        const [currentPartnerMember] = this.getRecords("mail.channel.member", [
            ["channel_id", "=", channel.id],
            ["partner_id", "=", this.pyEnv.currentPartnerId],
        ]);
        if (currentPartnerMember) {
            this.pyEnv["mail.channel.member"].write([currentPartnerMember.id], {
                is_pinned: true,
            });
        } else {
